...
class Binds:
    ...
    _direction_keys: dict[Bind, Direction] = {}
    ...
    def load_binds(cls) -> None:
        ...
        for category_name, category in hotkeys.items():
            for bind_name, binds in category.items():
                setattr(cls, bind_name, BindSequence(binds))
            category_binds = chain.from_iterable(binds for binds in category.values())
            setattr(cls, category_name, BindSequence(category_binds))
        cls.set_directions()

    @classmethod
    def set_directions(cls) -> None:
        def set_every_to(every: Iterable[Bind], to: Direction, at: MutableMapping) -> None:
            for item in every:
                at[item] = to
    ...
    @classmethod
    def undefined_keys(cls) -> Iterable[str]:
        return [key for key, value in vars(cls).items() if value is unbound]

    @classmethod
    def get_direction(cls, bind: Bind) -> Direction:
